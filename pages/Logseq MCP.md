...
5. Under **"Authorization tokens"** → add a new token with value `logseqbdBD1974`
6. Ensure `LOGSEQ_API_TOKEN=logseqbdBD1974` is set in your `.env`
...
logseq-mcp:
  ...
  environment:
    LOGSEQ_API_TOKEN: ${LOGSEQ_API_TOKEN:-logseqbdBD1974}
    MCP_HTTP_AUTH_TOKEN: ${MCP_HTTP_AUTH_TOKEN:-logseqbdBD1974}
    LOGSEQ_API_URL: http://host.docker.internal:12315
  command: >
    sh -c "pip install --quiet mcp-logseq &&
           mcp-logseq --transport http --host 0.0.0.0 --port 8001 --insecure"
  ports:
    - "8001:8001"
```
...
|-------|----------|-------|
| `--transport sse` not supported | `mcp-logseq --transport sse` | `--transport http` |
| HTTP transport requires auth token | missing | `MCP_HTTP_AUTH_TOKEN` added |
| HTTP on plain network refused | missing | `--insecure` flag added |
...
```json
{
  "mcpServers": {
    "logseq": {
      "url": "http://localhost:8001/mcp",
      "transport": "http",
      "headers": { "Authorization": "Bearer logseqbdBD1974" }
    }
  }
}
```
...
- **"Connection refused"** — Make sure Logseq is running and the HTTP API server is started (🔌 → Start)
- **"401 Unauthorized"** — `LOGSEQ_API_TOKEN` in `.env` must match the token you created in Logseq
- **Container keeps restarting with `invalid choice: sse`** — Old image; `--transport sse` was removed; use `--transport http --insecure`
- **`MCP_HTTP_AUTH_TOKEN is required`** — Add `MCP_HTTP_AUTH_TOKEN` env var to the container
