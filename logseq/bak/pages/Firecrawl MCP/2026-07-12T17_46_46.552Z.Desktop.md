...
firecrawl-mcp:
  image: node:22-slim
  environment:
    FIRECRAWL_API_KEY: ${FIRECRAWL_API_KEY:-fc-3c2677e6221e4893aff7f02eb5886b45}
    FIRECRAWL_API_URL: ${FIRECRAWL_API_URL:-https://api.firecrawl.dev}
    HTTP_STREAMABLE_SERVER: "true"
    HOST: "0.0.0.0"       # ← required fix (see below)
    PORT: "3000"
...
### ⚠️ Why `HOST: "0.0.0.0"` is required

By default, FastMCP (which firecrawl-mcp uses internally) binds to `127.0.0.1` (loopback) inside the container.
Docker's port-mapping proxy runs **outside** the container and cannot reach a loopback-bound service.
Without `HOST: "0.0.0.0"`, the port is listed as `LISTENING` on the host but every request gets an empty reply.
...
The server uses **FastMCP stateless HTTP stream** mode — every POST is handled independently with no session state.
...
- **Empty reply / fetch failed** — Missing `HOST: "0.0.0.0"` env var; server is binding to loopback only
- **Container shows `unhealthy`** — The healthcheck uses `wget` which is not in `node:22-slim`; harmless, server still works
